...
def plot_encodings_split(epoch, exp_directory, save=False, normalize=False, legend=False):
    ...
    for i, base in enumerate(base_tasks):
        ...
        for dim in range(latent_dim):
            ...
            if axes_tuple.shape[1] > 1:
                ...
                if len(nameWithoutVersion.split('-')) > 2:
                    ...
                    split_name = nameWithoutVersion
                split_name = number2name[base]
                axes_tuple[0][i].set_title(split_name, fontsize=16)
            else:
                axes_tuple[0][i].set_title("Epoch " + str(epoch), fontsize=16)
        ...
        axes_tuple[1][i].set_xlabel("Specification", fontsize=16)
        axes_tuple[1][i].set_ylim(-1, np.max(base_task_encodings) + 1)
        axes_tuple[0][i].tick_params(axis="x", labelsize=14)
        axes_tuple[0][i].tick_params(axis="y", labelsize=14)
        axes_tuple[1][i].tick_params(axis="x", labelsize=14)
        axes_tuple[1][i].tick_params(axis="y", labelsize=14)
        axes_tuple[1][i].set_yticks(np.arange(-1, np.max(base_task_encodings) + 2, 1))
        axes_tuple[0][i].grid()
        axes_tuple[1][i].grid(which='minor')
        axes_tuple[1][i].grid(which='major')
        axes_tuple[0][0].set_ylabel('Encoding $\mathbf{z}$', fontsize=16)
        axes_tuple[1][0].set_ylabel('Encoding $\mathbf{y}$', fontsize=16)
        plt.tight_layout()
    if legend:
