...
class HalfCheetahMixtureEnv(RandomEnv, utils.EzPickle):
    def __init__(self, *args, **kwargs):
        ...
        self.non_stationary_tasks = None

        self.model_path = os.path.join(os.getcwd(), 'meta_rand_envs', 'half_cheetah.xml')
    ...
    def setup_non_stationary_tasks(self):
        # TODO: Improve task list?
        self.non_stationary_tasks = []

        if len(self.task_variants) > 1:
            if 'stand_back' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['stand_back'], 'specification': np.float64(-np.pi / 5.), 'color': np.array([0.5, 0, 1.])})
            if 'goal_backward' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['goal_backward'], 'specification': np.float64(-5.), 'color': np.array([0, 1, 1])})
            if 'jump' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['jump'], 'specification': np.float64(2.), 'color': np.array([0.5, 0.5, 0.5])})
            if 'velocity_forward' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['velocity_forward'], 'specification': np.float64(2.0), 'color': np.array([1, 0, 0])})
            if 'stand_front' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['stand_front'], 'specification': np.float64(np.pi / 2.), 'color': np.array([1., 0, 0.5])})
            if 'goal_forward' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['goal_forward'], 'specification': np.float64(10.), 'color': np.array([1, 1, 0])})
            if 'velocity_backward' in self.task_variants:
                self.non_stationary_tasks.append({'base_task': self.bt2t['velocity_backward'], 'specification': np.float64(-3.), 'color': np.array([0, 1, 0])})
        else:
            if 'stand_front' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['stand_front'], 'specification': s, 'color': np.array([1., 0, 0.5])}
                                              for s in [np.float64(np.pi * 3. / 12.), np.float64(np.pi * 4. / 12.), np.float64(np.pi * 5. / 12.)]]
            if 'stand_back' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['stand_back'], 'specification': s, 'color': np.array([0.5, 0, 1.])}
                                              for s in [np.float64(-np.pi * 3. / 12.), np.float64(-np.pi * 4. / 12.), np.float64(-np.pi * 5. / 12.)]]

            if 'velocity_forward' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['velocity_forward'], 'specification': s, 'color': np.array([1, 0, 0])}
                                              for s in [np.float64(2.0), np.float64(3.0), np.float64(4.0)]]
            if 'velocity_backward' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['velocity_backward'], 'specification': s, 'color': np.array([0, 1, 0])}
                                              for s in [np.float64(-2.0), np.float64(-3.0), np.float64(-4.0)]]

            if 'goal_forward' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['goal_forward'], 'specification': s, 'color': np.array([1, 1, 0])}
                                              for s in [np.float64(5.), np.float64(12.5), np.float64(20.)]]
            if 'goal_backward' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['goal_backward'], 'specification': s, 'color': np.array([0, 1, 1])}
                                              for s in [np.float64(-5.), np.float64(-12.5), np.float64(-20.)]]

            if 'jump' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['jump'], 'specification': s, 'color': np.array([0.5, 0.5, 0.5])}
                                              for s in [np.float64(1.0), np.float64(2.0), np.float64(3.)]]
            if 'flip_forward' in self.task_variants:
                self.non_stationary_tasks += [{'base_task': self.bt2t['flip_forward'], 'specification': s, 'color': np.array([0.5, 0.5, 0])}
                                              for s in [np.float64(1. * np.pi), np.float64(2. * np.pi), np.float64(3. * np.pi)]]

        self.non_stationary_tasks = [np.array(self.non_stationary_tasks)[np.array(range(len(self.non_stationary_tasks)))] for _ in range(len(self.test_tasks))]

    def _step(self, action):
        # change task after some steps
        if self.change_mode == "time" and not self.initialize:

            if self.non_stationary_tasks is None: self.setup_non_stationary_tasks()
            ...
            if self.tasks[self.last_idx]['current_step'] % self.change_steps == 0:
                task_spec = self.non_stationary_tasks[self.last_idx][(self.tasks[self.last_idx]['current_step'] // self.change_steps) % len(self.non_stationary_tasks[self.last_idx])]
    ...
    def sample_tasks(self, num_tasks_list):
        ...
        num_tasks_per_subtask = [int(np.ceil(num_tasks / num_base_tasks)) for num_tasks in num_tasks_list]
        ...
        if 'velocity_forward' in self.task_variants:
            ...
            tasks_velocity = [{'base_task': self.bt2t['velocity_forward'], 'specification': velocity, 'color': np.array([1, 0, 0])} for velocity in velocities]
        ...
        if 'velocity_backward' in self.task_variants:
            ...
            tasks_velocity = [{'base_task': self.bt2t['velocity_backward'], 'specification': velocity, 'color': np.array([0, 1, 0])} for velocity in velocities]
            np.random.shuffle(tasks_velocity)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_velocity[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]
        ...
        if 'goal_forward' in self.task_variants:
            ...
            tasks_goal = [{'base_task': self.bt2t['goal_forward'], 'specification': goal, 'color': np.array([1,1,0])} for goal in goals]
        ...
        if 'goal_backward' in self.task_variants:
            ...
            tasks_goal = [{'base_task': self.bt2t['goal_backward'], 'specification': goal, 'color': np.array([0, 1, 1])} for goal in goals]
            np.random.shuffle(tasks_goal)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_goal[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]
        ...
        if 'stand_front' in self.task_variants:
            ...
            tasks_stand_up = [{'base_task': self.bt2t['stand_front'], 'specification': goal, 'color': np.array([1., 0, 0.5])} for goal in goals]
            np.random.shuffle(tasks_stand_up)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_stand_up[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]

        if 'stand_back' in self.task_variants:
            goals = np.linspace(-self.pos_y[1], -self.pos_y[0], num=sum(num_tasks_per_subtask))
            tasks_stand_up = [{'base_task': self.bt2t['stand_back'], 'specification': goal, 'color': np.array([0.5, 0, 1.])} for goal in goals]
            np.random.shuffle(tasks_stand_up)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_stand_up[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]
        ...
        if 'flip_forward' in self.task_variants:
            ...
            tasks_flipping = [{'base_task': self.bt2t['flip_forward'], 'specification': goal, 'color': np.array([0.5, 0.5, 0])} for goal in goals]
            np.random.shuffle(tasks_flipping)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_flipping[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]

        # direction
        if 'direction_forward' in self.task_variants:
            goals = np.array([1.] * sum(num_tasks_per_subtask))
            tasks_jump = [{'base_task': self.bt2t['direction_forward'], 'specification': goal, 'color': np.array([0.5, 0.5, 0.])} for goal in goals]
            np.random.shuffle(tasks_jump)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_jump[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]
        if 'direction_backward' in self.task_variants:
            goals = np.array([-1.] * sum(num_tasks_per_subtask))
            tasks_jump = [{'base_task': self.bt2t['direction_backward'], 'specification': goal, 'color': np.array([0.5, 0., 0.5])} for goal in goals]
            np.random.shuffle(tasks_jump)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_jump[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]
        if 'velocity' in self.task_variants:
            goals = np.linspace(0.0, 3.0, num=sum(num_tasks_per_subtask))
            tasks_jump = [{'base_task': self.bt2t['velocity'], 'specification': goal, 'color': np.array([0.5, 0., 0.5])} for goal in goals]
            np.random.shuffle(tasks_jump)
            for i in range(len(num_tasks_list)): tasks[i] += tasks_jump[num_tasks_per_subtask_cumsum[i-1] if i - 1 >= 0 else 0:num_tasks_per_subtask_cumsum[i]]
